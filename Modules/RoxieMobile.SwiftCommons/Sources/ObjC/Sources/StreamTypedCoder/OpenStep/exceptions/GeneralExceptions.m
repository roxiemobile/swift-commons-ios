...
- __init
{
    return [self initWithName:NSMallocException reason:@"Memory exhausted" userInfo:nil];
}
...
- setPointer:(void**)_pointer memorySize:(unsigned)_size
{
    pointer = _pointer;
    size = _size;
    return [self initWithName:NSMallocException reason:@"Deallocation error" userInfo:nil];
}
